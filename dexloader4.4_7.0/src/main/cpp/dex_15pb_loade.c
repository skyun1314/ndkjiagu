...
#define JNIREG_CLASS "com/bluelesson/dummydex/DummyApplication"
...
void myExtractFile(JNIEnv* env,jobject application_obj,const char* szPath)
{
    ...
    if(access(szPath,R_OK))
    {
        ...
        AAsset* asset = AAssetManager_open(mgr, "src.dex",AASSET_MODE_STREAMING);
        FILE* file;
        void* buffer;
        int numBytesRead;
        if(asset)
...
void myLoadDex_dvm(JNIEnv* env,char* szPath)
{
    ...
    LOGI("sizeof ArrayObject:%d",(u4)sizeof(ArrayObject));
    ao->length=dexLen;
...
void replace_classloader_cookie(JNIEnv *env,jobject classLoader)
{
    if(sdk_int>13)
    {
        ...
        while(i<count)
        {
            ...
            {
                LOGI("SetObjectField");
                LOGI("---art_MarCookie:%X",(u4)art_MarCookie);
