...
int find_name(pid_t pid, const char *name, const char *libn,
    ...
    if (0 > find_libname((char *) libn, (char *) libc, sizeof(libc), &libcaddr, mm, nmm)) {
...
static int enumlookup(struct symlist *sl, unsigned char type,
                      std::function<bool(const char *name)> name_callback) {
    Elf32_Sym *p;
    int i;
...
extern int
enum_func(pid_t pid, const char *libn, std::function<bool(const char *name)> name_callback) {
    struct mm mm[1000] = {0};
    unsigned long libcaddr;
    int nmm;
    char libc[1024] = {0};
