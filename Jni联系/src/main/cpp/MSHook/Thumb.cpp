...
void Thumb::SubstrateHookFunctionThumb(SubstrateProcessRef process, void *symbol, void *replace, void **result){
    ...
    if (
       ...
       // arm[1] = reinterpret_cast<uint32_t>(replace);
        arm[1] = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(replace));
        return;
    }
    ...
    for (unsigned offset(0); offset != used / sizeof(uint16_t); ++offset) {
        if (T$pcrel$ldr(backup[offset])) {
            ...
            *--trailer = ((  static_cast<uint32_t>(reinterpret_cast<uintptr_t>(area + offset))+ 4) & ~0x2) + bits.immediate * 4;
            start += 2;
            end -= 2;
        } else if (T$pcrel$b(backup[offset])) {
            union {
            ...
            *--trailer =static_cast<uint32_t>(reinterpret_cast<uintptr_t>(area + offset)) + 4 + jump;
            ...
            *--trailer = (( static_cast<uint32_t>(reinterpret_cast<uintptr_t>(area + offset)) + 4) & ~0x2) + (bits.u == 0 ? -exts.immediate : exts.immediate);
            ++offset;
            start += 4;
            end -= 2;
        } else if (T$pcrel$add(backup[offset])) {
            ...
            *--trailer = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(area + offset))+ 4;
            start += 5;
            end -= 2;
        } else if (T$32bit$i(backup[offset])) {
   ...
   // transfer[1] = reinterpret_cast<uint32_t>(area + used / sizeof(uint16_t)) + 1;
        transfer[1] =   static_cast<uint32_t>(reinterpret_cast<uintptr_t>(area + used / sizeof(uint16_t)))+ 1;
